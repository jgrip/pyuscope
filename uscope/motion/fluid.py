...
class Timeout(FluidException):
    pass


class HomingFailed(FluidException):
    pass


def default_port():
    port = os.getenv("FLUID_PORT", None)
    if port:
        return port
    ...
    ports = glob.glob(
        "/dev/serial/by-id/usb-Silicon_Labs_CP2102_USB_to_UART_Bridge_Controller_0001-*"
    )
    if len(ports) == 0:
        raise Exception("Failed to auto detect any FluidNC serial ports")
    if len(ports) == 1:
        return ports[0]
    raise Exception("Need explicit FLUID serial port (ie FLUID_PORT=/dev/blah)")
...
class FluidSer:
    def __init__(
        ...
        self.verbose = (
            ...
            else bool(int(os.getenv("FLUIDSER_VERBOSE", "0")))
        )
        ...
        self.verbose and print(
            "opening %s in thread %s" % (port, threading.get_ident())
        )
...
"""
Emulate a FluidNC serial port for testng on the go
"""


class MockFluidSer(FluidSer):
    STATE_RESET = None
    STATE_IDLE = "Idle"
...
class Fluid:
    def __init__(
        ...
        self.verbose = (
            verbose
            if verbose is not None
            else bool(int(os.getenv("FLUID_VERBOSE", "0")))
        )
        self.port = None
        if gs is None:
            if port == "mock":
                gs = MockFluidSer(verbose=verbose)
            else:
                gs = FluidSer(port=port, verbose=verbose)
                self.port = port
        assert gs
        self.gs = gs
        if flush:
        ...
        # See move_relative
        self.use_soft_move_relative = int(os.getenv("FLUID_SOFT_RELATIVE", "1"))
    ...
    def reset_probe(self):
        ...
        if not self.gs.in_reset():
            return
        print("fluid: failed to respond, attempting reset recovery")
        self.gs.reset_recover()

        # Run full status command to be sure
        self.qstatus()
        # grbl: recovered after 1.388 sec
        print("fluid: recovered after %0.3f sec" % (time.time() - tbegin,))
        ...
        for tryi in range(tries):
            try:
                ...
                if not retry:
                    raise
                if tryi == tries - 2:
                    raise MotionCritical("Unable to recover FluidNC comms")
                continue
    ...
    def move_absolute(self, pos, f, blocking=True):
        ...
        for i in range(tries):
            try:
                # implies G1
                ax_str = "".join([" %c%0.3f" % (k.upper(), v) for k, v in pos.items()])
        ...
        try:
            for axis, scalar in scalars.items():
                ...
                if 0 and self.verbose:
                    ...
                    print(
                        "jog: X%0.3f Y%0.3f Z%0.3F" % (mpos["x"], mpos["y"], mpos["z"])
                    )
        except Timeout:
...
class FluidHal(MotionHAL):
    def __init__(self, verbose=None, port=None, fluid=None, **kwargs):
        self.grbl = None
        self.feedrate = None
        if fluid:
            self.fluid = fluid
        else:
            self.fluid = Fluid(port=port, verbose=verbose)
...
def get_fluid(port=None, gs=None, reset=False, verbose=False):
    return Fluid(port=port, gs=gs, reset=reset, verbose=verbose)
